...
在某些系统中可以创建一个作为文件系统里真实的命名文件而存在的长时间运行的管道，这种文件叫做命名管道（*FIFO*）。它可以用作线程、进程及独立启动的程序间的*IPC*机制。*FIFO*管道更适合作为独立客户端和服务器程序的一般*IPC*机制，它是套接字端口的替代机制，但**目前不在*Windows*下*Python*标准版本中**。
